...
from prompts.promptsFirst import PROMPT_JSON as PROMPT, PROMPT_VALIDATION

# First iteration, future iterations will also include

projectName = project_Name
exePreprocessor = preprocessor(projectName)
exeJson = exePreprocessor.getJsonName()
if exeJson and not os.path.isfile(exeJson): 
    exePreprocessor.preprocess()
...
for currKey in keys:
    exeData = data(currKey)
    check, ruleName, ruleRiskDescription, ruleRiskAssessment, ruleRiskSolution, componentPath, lineStart, snippetStart, snippetEnd, snippet = exeData.get()
    if check == 0:
